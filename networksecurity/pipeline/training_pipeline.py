...
class TrainingPipeline():
    ...
    def run_pipeline(self):
        try:
            ...
            logger.info(f"{model_trainer_artifact}")
            logger.info("<<Training Pipeline Ended>>>>>>>>>>>>>>")

            artifacts_bucket_url = self.sync_artifact_dir_to_s3()
            logger.info(f"Artifacts saved at {artifacts_bucket_url}")
            model_bucket_url = self.sync_saved_model_to_s3()
            logger.info(f"Model saved at {model_bucket_url}")
