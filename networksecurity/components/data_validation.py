...
class DataValidation:
    ...
    def validate_num_of_columns(self, df:pd.DataFrame):
        try:
            number_of_columns = len(self.scheme_config)
    ...
    def initiate_data_validation(self):
        try:
            ...
            status = self.validate_num_of_columns(df=train_dataframe)
            if not status:
                error_message = f"Train Dataframe Does not Contain All Columns.\n"
            status = self.validate_num_of_columns(df=test_dataframe)
            if not status:
